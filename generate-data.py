...
def get(
    ...
    sorted_signatures = sorted(
        signatures.items(), key=lambda x: x[1][0][0], reverse=True
    )
    i = 1
...
def get_with_retries(url, params=None, headers=None):
    ...
    s = requests.Session()
    s.mount(
        "https://analysis-output.telemetry.mozilla.org",
...
if __name__ == "__main__":
    ...
    Bugzilla.TOKEN = os.environ.get("BUGZILLA_TOKEN")
    ...
    for channel in ["release", "beta"]:
        for startup in [False, True]:
            ...
            with open(
                "dist/" + channel + ("-startup" if startup else "") + ".json", "w"
            ) as f:
